...
class Deviation {
    ...
    function getDeviatingStatistics() {
        ...
        uasort($this->results, ['statisticFunctions\functions\Deviation', 'orderByLength']);
    ...
    static function removeOutliers($results) {
        ...
        function orderAllResults() {
            foreach ($this->resultsWithTime as $result => $operation) {
                ...
                if ($result !== "") {
                    ...
                    if(is_numeric($stats_standard_dev)) {
                         $this->results[$result] =
                            $this->addOperationInformation($operation, $amount, $meanTime, $stats_standard_dev,
                                                         $percentage);
                    }
                }
            }
         ...
         */
        private
        function addOperationInformation($results, $amount, $meanTime, $stats_standard_dev, $percentage) {
            $result = $results;
         ...
         */
        private
        static function orderByLength($a, $b) {
            if ($a["mean"] == $b["mean"]) {
                return 0;
            }
